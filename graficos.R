...
color=c("lightsalmon4","lightsteelblue2")
...
# 1.- Creación de un vector de color
color=c("darkseagreen2", "lavenderblush3", "lemonchiffon2")
...
GB2<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "pink", fill=color)+
  ggtitle("Gráfico de Barras")+
  xlab("Año")+
  ylab("Frecuencias")+
  coord_flip()+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="pink", fill="paleturquoise")+
  ggtitle("Histograma")+
